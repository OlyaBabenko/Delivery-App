...
const PageCart = () => {
    ...
    const imageUrl = getShopsImage(cart) //get src logo restaurant
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const responseToken = await axios.get('https://yyd-back.herokuapp.com/products/token/'); // Get token
            const requestOptions = {
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': responseToken.data.token,
                }}
            const responseData = await axios.post('https://yyd-back.herokuapp.com/products/order/', formData, requestOptions); // POST order
            ...
            {!isCartEmpty 
                ? (<div className={style.cartContent}>    
                    ...
                    <div className={style.cart}>
                        ...
                        <h3 className={style.totalPrice}>Total price: {totalPrice} ₴</h3>
                    </div>                                   
                </div>)
             : <div className={style.cart} style={{ display: 'flex', alignItems: "center"}}>
